...
def _huffman_coding_per_layer(weight, centers):
    ...
    for i in range(3,len(sorted_freq),1):
        ...
        if i + 1 < len(sorted_freq):
            ...
            if node1[1] >= huffman_tree[pos-1] and temp_tree[2] == 0:
                ...
                old_freq = temp_tree[2]
                
                
                if len(temp_trees) > 1 and temp_tree[2] >= huffman_tree[pos - 1]:
                    
                    for tree in temp_trees
                    
                    temp_trees = []
                
                elif temp_tree[2] >= huffman_tree[pos - 1]:#len(temp_trees) == 0:
                    huffman_tree.append(temp_tree[0])
                    huffman_tree.append(temp_tree[1])
                    huffman_tree.append( temp_tree[2] + huffman_tree[pos - 1] )
                    pos +=3
                ...
                if len(temp_trees) > 1 and temp_tree[2] < huffman_tree[pos - 1]:
                    temp_tree[2] = temp_tree[2] + old_freq
