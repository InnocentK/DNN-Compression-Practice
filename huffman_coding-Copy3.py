...
def _huffman_coding_per_layer(weight, centers):
    ...
    #max_tree_size = (len(sorted_freq) * 2) - 1
    #huffman_tree = [None] * 3
    ...
    for i in range(3,len(sorted_freq),1):
        print(huffman_tree)
        if i + 1 < len(sorted_freq):
            node1 = sorted_freq[i] 
            node2 = sorted_freq[i + 1]
            print(node1[1] >= huffman_tree[pos-1], node1[1], huffman_tree[pos-1])
            
            # Can add node
            if node1[1] >= huffman_tree[pos-1]:# and len(temp_trees) == 0:
                huffman_tree.append(node1)
                huffman_tree.append( huffman_tree[pos-1] + node1[1] )
                pos += 2
                
                # Can add previous trees
                if max_temp_freq >= huffman_tree[pos-1]:
                    
                    #add trees if needed
                    for temp_tree in temp_trees:
                        
                        huffman_tree.append(temp_tree[0])
                        huffman_tree.append(temp_tree[1])
                        huffman_tree.append(temp_tree[2])
                        pos += 3
                    
                    #empty out temp trees
                    temp_trees = []

            # Can't add node            
            else:
                temp_tree = [node1, node2, node1[1] + node2[1]]
                temp_trees.append(temp_tree)
                #max_temp_freq += temp_tree[2]
